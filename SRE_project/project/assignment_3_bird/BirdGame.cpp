...
void BirdGame::init() {
    ...
    // add floor/ceiling to prevent the bird from going out of bounds
    auto floor = createGameObject();
    floor->name = "Floor";
    auto floorPhys = floor->addComponent<PhysicsComponent>();
    float floorWidth = 100000.;
    float floorHeight = 87.5;
    floor->setPosition(glm::vec2(floorWidth/2, floorHeight/2));
    floorPhys->initBox(b2_staticBody, glm::vec2(floorWidth/physicsScale, floorHeight/physicsScale), glm::vec2(floor->getPosition().x/physicsScale, floor->getPosition().y/physicsScale), 0);
    floorPhys->fixture->SetFriction(0.);
    auto ceiling = createGameObject();
    ceiling->name = "Ceiling";
    auto ceilPhys = ceiling->addComponent<PhysicsComponent>();
    float ceilWidth = 100000.;
    float ceilHeight = 50.;
    ceiling->setPosition(glm::vec2(ceilWidth/2, windowSize.y + ceilHeight));
    ceilPhys->initBox(b2_staticBody, glm::vec2(ceilWidth/physicsScale, ceilHeight/physicsScale), glm::vec2(ceiling->getPosition().x/physicsScale, ceiling->getPosition().y/physicsScale), 1);
    ceilPhys->fixture->SetFriction(0.);
    ...
    for (int i=0;i<length;i++) {
        ...
        phys->initBox(b2_staticBody, s/physicsScale, glm::vec2(obj->getPosition().x/physicsScale, obj->getPosition().y/physicsScale), 1);
    }
    ...
    spriteCoin.setScale({1.5,1.5});
    for(int i = 0; i < length; i++) {
        auto obj = createGameObject();
        obj->name = "Coin";
        auto sprite = obj->addComponent<SpriteComponent>();
        float xOffset = xVariation * cos(i*curve*0.2f);
        glm::vec2 pos{i*300+xOffset, (windowSize.y-spriteCoin.getSpriteSize().y/2 + 2*sin(i*curve)*heighVariation + spriteCoin.getSpriteSize().y/2)/2};
        obj->setPosition(pos);
        sprite->setSprite(spriteCoin);
        glm::vec2 s { spriteCoin.getSpriteSize().x * spriteCoin.getScale().x/2, spriteCoin.getSpriteSize().y * spriteCoin.getScale().y/2};
        auto phys = obj->addComponent<PhysicsComponent>();
        phys->initCircle(b2_staticBody, 10/physicsScale, glm::vec2(obj->getPosition().x/physicsScale, obj->getPosition().y/physicsScale), 1);
        phys->setSensor(true);
    }
