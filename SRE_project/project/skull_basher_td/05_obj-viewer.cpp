...
#include "architecture/Model.hpp"
...
public:
    ObjViewerExample(){
        ...
        material->setSpecularity(Color(1,1,1,0.0f));
        ...
        model = Model::create().withMesh(mesh).withMaterials(materials).build();
//        model = Model::create().withOBJ(".\\assets\\models\\BarrelD.obj").withMaterials(materials).build();
    ...
    void loadObjFile(std::string file){
        ...
        materials.clear();
        model = Model::create().withOBJ(file).build();
    ...
    void render(){
        auto renderPass = RenderPass::create()
                ...
                .build();
        model->setTransform(glm::scale(glm::vec3(0.5f))*glm::eulerAngleY(glm::radians((float)i))*glm::translate(offset));
        renderPass.draw(model->getMesh(), model->getTransform(), model->getMaterials());
